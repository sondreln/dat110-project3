...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		...
		if (Util.checkInterval(key, nodeID.add(BigInteger.ONE), succID) || key.equals(succID)) {
			return succ;
		} else {
	...
	private NodeInterface findHighestPredecessor(BigInteger ID) throws RemoteException {
        ...
        BigInteger nodeId = node.getNodeID(); 
        BigInteger idPlusOne = nodeId.add(BigInteger.ONE);
        ...
        for(int i =  fingerTable.size() - 1; i>= 0; i--){
            ...
            if(Util.checkInterval(fingerId, idPlusOne, ID.subtract(BigInteger.ONE))){
